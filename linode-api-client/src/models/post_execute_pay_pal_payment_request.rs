/*
 * Akamai: Linode API
 *
 * Add a Cloud Computing instance so you can build, release, and scale applications faster with virtual machines. 
 *
 * The version of the OpenAPI document: 4.193.0
 * Contact: jperez@linode.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// PostExecutePayPalPaymentRequest : An object representing an execution of Payment to PayPal to capture the funds and credit your Linode Account.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct PostExecutePayPalPaymentRequest {
    /// The PayerID returned by PayPal during the transaction authorization process.
    #[serde(rename = "payer_id")]
    pub payer_id: String,
    /// The PaymentID returned from [Stage a PayPal payment](https://techdocs.akamai.com/linode-api/reference/post-pay-pal-payment) that has been approved with PayPal.
    #[serde(rename = "payment_id")]
    pub payment_id: String,
}

impl PostExecutePayPalPaymentRequest {
    /// An object representing an execution of Payment to PayPal to capture the funds and credit your Linode Account.
    pub fn new(payer_id: String, payment_id: String) -> PostExecutePayPalPaymentRequest {
        PostExecutePayPalPaymentRequest {
            payer_id,
            payment_id,
        }
    }
}

