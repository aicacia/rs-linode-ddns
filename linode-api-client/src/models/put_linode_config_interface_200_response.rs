...
/// PutLinodeConfigInterface200Response : The network interface to apply to this Linode's configuration profile.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct PutLinodeConfigInterface200Response {
    /// __Read-only__ Returned as `true` if the interface has been booted using the configuration profile to which the interface belongs.
    ...
    pub ipv4: Option<models::PutLinodeConfigInterface200ResponseIpv4>,
    /// __Filterable__ The name set for an interface with a `purpose` of `vlan`. Returned as `null` if the interface has a `purpose` of `public` or `vpc`.
    #[serde(rename = "label", default, with = "::serde_with::rust::double_option", skip_serializing_if = "Option::is_none")]
    pub label: Option<Option<String>>,
    /// The default route to the Linode. Each Linode can have one interface set as its `primary`. Defaults to the first non-`vlan` type interface, if you haven't specifically set a `primary`.  > 📘 > > The value needs to `false` for any interface that uses `vlan` as its `purpose`.
    ...
    /// The `id` of the VPC subnet for this interface. Only applies to interfaces with a `purpose` of `vpc`. Returned as `null` if the interface has a `purpose` of `public` or `vlan`.  > 📘 > > You need to reboot a Linode using the interface's configuration profile to assign the Linode to a VPC subnet.
...
impl PutLinodeConfigInterface200Response {
    /// The network interface to apply to this Linode's configuration profile.
    pub fn new(purpose: PurposeEnum) -> PutLinodeConfigInterface200Response {
