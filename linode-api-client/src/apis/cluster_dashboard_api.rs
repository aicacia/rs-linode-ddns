...
/// struct for typed errors of method [`get_lke_cluster_dashboard`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetLkeClusterDashboardError {
...
pub async fn get_lke_cluster_dashboard(configuration: &configuration::Configuration, api_version: &str, cluster_id: i32) -> Result<models::GetLkeClusterDashboard200Response, Error<GetLkeClusterDashboardError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_api_version = api_version;
    let p_cluster_id = cluster_id;

    let uri_str = format!("{}/{apiVersion}/lke/clusters/{clusterId}/dashboard", configuration.base_path, apiVersion=crate::apis::urlencode(p_api_version), clusterId=p_cluster_id);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetLkeClusterDashboardError> = serde_json::from_str(&content).ok();
