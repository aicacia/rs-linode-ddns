...
/// struct for typed errors of method [`get_ipv6_pools`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum GetIpv6PoolsError {
...
pub async fn get_ipv6_pools(configuration: &configuration::Configuration, api_version: &str, page: Option<i32>, page_size: Option<i32>) -> Result<models::GetIpv6Pools200Response, Error<GetIpv6PoolsError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_api_version = api_version;
    let p_page = page;
    let p_page_size = page_size;

    let uri_str = format!("{}/{apiVersion}/networking/ipv6/pools", configuration.base_path, apiVersion=crate::apis::urlencode(p_api_version));
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<GetIpv6PoolsError> = serde_json::from_str(&content).ok();
